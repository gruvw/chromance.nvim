...
---@class chromance.util.theme
local M = {}

local function load_autocmds()
  local ok, _ = pcall(require, "chromance.autocmds")
  if not ok then
    Util.log("Failed to load chromance.autocmds", "error")
  end
end
...
function M.load(hl_group_tbl)
  ...
  local bufferline_icon_hl_group_tbl = require("chromance.theme.plugins.bufferline").setup_bufferline_icon()
